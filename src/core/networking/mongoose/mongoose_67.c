...
int mg_printf_(struct mg_connection *conn, const char *fmt, ...) {
  int len;
  va_list ap;
  va_start(ap, fmt);
...
void mg_if_recv_udp_cb(struct mg_connection *nc, void *buf, int len,
  ...
  if (nc->flags & MG_F_LISTENING) {
     ...
     */
    for (nc = mg_next_(lc->mgr, NULL); nc != NULL; nc = mg_next_(lc->mgr, nc)) {
...
struct mg_connection *mg_connect_(struct mg_mgr *mgr, const char *address,
                                 mg_event_handler_t callback) {
...
struct mg_connection *mg_next_(struct mg_mgr *s, struct mg_connection *conn) {
...
void mg_forward_(struct mg_connection *from, struct mg_connection *to) {
...
static void mg_mgr_handle_ctl_sock(struct mg_mgr *mgr) {
  ...
  if (len >= (int) sizeof(ctl_msg.callback) && ctl_msg.callback != NULL) {
    struct mg_connection *nc;
    for (nc = mg_next_(mgr, NULL); nc != NULL; nc = mg_next_(mgr, nc)) {
...
void mg_send_head(struct mg_connection *c, int status_code,
  ...
  if (content_length < 0) {
    mg_printf_(c, "%s", "Transfer-Encoding: chunked\r\n");
  } else {
    mg_printf_(c, "Content-Length: %" INT64_FMT "\r\n", content_length);
  }
  mg_send(c, "\r\n", 2);
}
...
static void mg_http_serve_file2(struct mg_connection *nc, const char *path,
                                ...
                                struct mg_serve_http_opts *opts) {
#if MG_ENABLE_HTTP_SSI
  if (mg_match_prefix_(opts->ssi_pattern, strlen(opts->ssi_pattern), path) > 0) {
...
int mg_url_decode_(const char *src, int src_len, char *dst, int dst_len,
  ...
  if (dst == NULL || dst_len == 0) {
    ...
    for (p = buf->p; p + name_len < e; p++) {
      if ((p == buf->p || p[-1] == '&') && p[name_len] == '=' &&
        ...
        len = mg_url_decode_(p, (size_t)(s - p), dst, dst_len, 1);
        if (len == -1) {
          len = -2;
        }
        break;
...
static int mg_is_file_hidden(const char *path,
  ...
  return (exclude_specials && (!strcmp(path, ".") || !strcmp(path, ".."))) ||
         (p1 != NULL &&
          mg_match_prefix_(p1, strlen(p1), path) == (int) strlen(p1)) ||
         (p2 != NULL && mg_match_prefix_(p2, strlen(p2), path) > 0);
}
...
void mg_http_reverse_proxy(struct mg_connection *nc,
  ...
  mg_printf_(be, "%.*s %s HTTP/1.1\r\n", (int) hm->method.len, hm->method.p,
            path);

  mg_printf_(be, "Host: %s\r\n", addr);
  for (i = 0; i < MG_MAX_HTTP_HEADERS && hm->header_names[i].len > 0; i++) {
    ...
    if (mg_vcasecmp(&hn, "Transfer-encoding") == 0 &&
      ...
      mg_printf_(be, "Content-Length: %" SIZE_T_FMT "\r\n", hm->body.len);
      continue;
    }
    ...
    *p = '\0';
    ...
    while (u <= cp_end) {
      ...
      if (component.len > 0) {
        ...
        len = mg_url_decode_(p + 1, component.len, p + 1, lp_end - p - 1, 0);
        if (len <= 0) {
          ok = 0;
          break;
        }
...
MG_INTERNAL void mg_send_http_file(struct mg_connection *nc, char *path,
  ...
  is_cgi =
      (mg_match_prefix_(opts->cgi_file_pattern, strlen(opts->cgi_file_pattern),
  ...
  switch (ev) {
    case MG_EV_HTTP_PART_BEGIN: {
      ...
      if (fus->fp == NULL) {
        ...
        mg_printf_(nc, "Failed to open %s: %d\n", fus->lfn, mg_get_errno());
      ...
      if (fwrite(mp->data.p, 1, mp->data.len, fus->fp) != mp->data.len) {
        ...
        if (mg_get_errno() == ENOSPC
          ...
          mg_printf_(nc,
                    ...
                    "Content-Type: text/plain\r\n"
                    "Connection: close\r\n\r\n");
          mg_printf_(nc, "Failed to write to %s: no space left; wrote %d\r\n",
          ...
          mg_printf_(nc,
                    ...
                    "Content-Type: text/plain\r\n"
                    "Connection: close\r\n\r\n");
          mg_printf_(nc, "Failed to write to %s: %d, wrote %d", mp->file_name,
                    mg_get_errno(), (int) fus->num_recd);
        }
        fclose(fus->fp);
        remove(fus->lfn);
...
size_t mg_parse_multipart_(const char *buf, size_t buf_len, char *var_name,
      ...
      if (nc->flags & MG_F_USER_1) {
        ...
        if (len < 0 || io->len > MG_MAX_HTTP_REQUEST_SIZE) {
          ...
          if (mg_get_http_header(&hm, "Location") != NULL) {
            mg_printf_(nc, "%s", "HTTP/1.1 302 Moved\r\n");
          } else if ((h = mg_get_http_header(&hm, "Status")) != NULL) {
            mg_printf_(nc, "HTTP/1.1 %.*s\r\n", (int) h->len, h->p);
          } else {
            mg_printf_(nc, "%s", "HTTP/1.1 200 OK\r\n");
          }
        }
...
static void mg_do_ssi_include(struct mg_connection *nc, struct http_message *hm,
  ...
  if (sscanf(tag, " virtual=\"%[^\"]\"", file_name) == 1) {
    ...
    mg_printf_(nc, "Bad SSI #include: [%s]", tag);
    return;
  }

  if ((fp = mg_fopen(path, "rb")) == NULL) {
    mg_printf_(nc, "SSI include error: mg_fopen(%s): %s", path,
              strerror(mg_get_errno()));
  } else {
    mg_set_close_on_exec((sock_t) fileno(fp));
    if (mg_match_prefix_(opts->ssi_pattern, strlen(opts->ssi_pattern), path) >
        0) {
...
static void do_ssi_exec(struct mg_connection *nc, char *tag) {
  ...
  if (sscanf(tag, " \"%[^\"]\"", cmd) != 1) {
    mg_printf_(nc, "Bad SSI #exec: [%s]", tag);
  } else if ((fp = popen(cmd, "r")) == NULL) {
    mg_printf_(nc, "Cannot SSI #exec: [%s]: %s", cmd, strerror(mg_get_errno()));
  } else {
    mg_send_file_data(nc, fp);
    pclose(fp);
  }
...
static void mg_send_ssi_file(struct mg_connection *nc, struct http_message *hm,
  ...
  if (include_level > 10) {
    mg_printf_(nc, "SSI #include level is too deep (%s)", path);
    return;
  }
  ...
  while ((ch = fgetc(fp)) != EOF) {
    if (in_ssi_tag && ch == '>' && buf[len - 1] == '-' && buf[len - 2] == '-') {
      ...
      if (len == (int) btag.len && strncmp(buf, btag.p, btag.len) != 0) {
        ...
        mg_printf_(nc, "%s: SSI tag is too large", path);
        len = 0;
      }
      buf[len++] = ch & 0xff;
    } else {
...
MG_INTERNAL void mg_handle_propfind(struct mg_connection *nc, const char *path,
  ...
  if (S_ISDIR(stp->st_mode) &&
    ...
    mg_printf_(nc, "%s", "HTTP/1.1 403 Directory Listing Denied\r\n\r\n");
  } else {
...
MG_INTERNAL void mg_handle_lock(struct mg_connection *nc, const char *path) {
  static const char *reply =
      ...
      "</D:lockdiscovery>"
      "</d:multistatus>\n";
  mg_printf_(nc, reply, path, (unsigned int) mg_time());
  nc->flags |= MG_F_SEND_AND_CLOSE;
}
#endif
...
void mg_send_websocket_handshake3(struct mg_connection *nc, const char *path,
  ...
  if (host != MG_WS_NO_HOST_HEADER_MAGIC) {
    mg_printf_(nc, "Host: %s\r\n", host);
  }
  if (protocol != NULL) {
    mg_printf_(nc, "Sec-WebSocket-Protocol: %s\r\n", protocol);
  }
  if (extra_headers != NULL) {
    mg_printf_(nc, "%s", extra_headers);
  }
  mg_printf_(nc, "\r\n");
...
int mg_match_prefix_(const char *pattern, int pattern_len, const char *str) {
...
int mg_resolve_async_opt(struct mg_mgr *mgr, const char *name, int query,
  ...
  dns_nc = mg_connect_(mgr, nameserver, mg_resolve_async_eh);
  if (dns_nc == NULL) {
    free(req);
    return -1;
  }
