...
class VRMLLoader {
    private:
        ...
        void applyPose() {
            VRTransformPtr t = dynamic_pointer_cast<VRTransform>(ctx.currentNode->obj);
            t->setMatrix(ctx.pose);
        }

        void applyMaterial() {
            VRGeometryPtr g = dynamic_pointer_cast<VRGeometry>(ctx.currentNode->obj);
            if (g && ctx.mat) g->setMaterial(ctx.mat);
        }
        ...
        void handleAsciiText(map<string, string> data) {
            ...
            VRSpritePtr s = dynamic_pointer_cast<VRSprite>(ctx.currentNode->obj);
        ...
        void handleCone(map<string, string> data) {
            ...
            VRGeometryPtr geo = dynamic_pointer_cast<VRGeometry>(ctx.currentNode->obj);
        ...
        void handlePointLight(map<string, string> data) {
            ...
            VRLightPtr light = dynamic_pointer_cast<VRLight>(ctx.currentNode->obj);
        ...
        void handlePerspectiveCamera(map<string, string> data) {
            ...
            VRCameraPtr c = dynamic_pointer_cast<VRCamera>(ctx.currentNode->obj);
        ...
        void handleTranslation(map<string, string> data) {
            Matrix4d m; m.setTranslate( getSFVec3f(data, "translation", Vec3d(0,0,0)) );
            ctx.pose.mult(m);
        }

        void handleRotation(map<string, string> data) {
            Vec4d rotation = getSFRotation(data, "rotation", Vec4d(0,0,1,0));
            Matrix4d m; m.setRotate( Quaterniond( Vec3d(rotation[0], rotation[1], rotation[2]), rotation[3] ) );
            ctx.pose.mult(m);
        }

        void handleScale(map<string, string> data) {
            Matrix4d m; m.setScale( getSFVec3f(data, "scaleFactor", Vec3d(1,1,1)) );
            ctx.pose.mult(m);
        }
        ...
        void handleCoordinate3(map<string, string> data) {
            ...
            for (auto p : positions) ctx.currentNode->geo.pushVert(Pnt3d(p));
        }

        void handleNormal(map<string, string> data) {
            auto normals = getMFVec3f(data, "normal", {});
            for (auto n : normals) ctx.currentNode->geo.pushNorm(n);
        }
        ...
        void handleIndexedFaceSet(map<string, string> data) {
            ...
            for (auto i : coordIndex) {
                if (i == -1) {
                    ...
                    if (face.size() == 3) ctx.currentNode->geo.pushTri(face[0], face[1], face[2]);
                    if (face.size() == 4) ctx.currentNode->geo.pushQuad(face[0], face[1], face[2], face[3]);
                    face.clear();
                    continue;
                }
                face.push_back(i);
            ...
            VRGeometryPtr geo = dynamic_pointer_cast<VRGeometry>(ctx.currentNode->obj);
            ctx.currentNode->geo.apply(geo);
            ctx.currentNode->geo = VRGeoData();
        }
        ...
        VRObjectPtr makeObject() {
            string type = ctx.currentNode->type;
            string name = ctx.currentNode->name;
        ...
        void handleGroupNode(string node) {
            if (node == "Separator") { // reset everything
        ...
        void applyProperties() {
            ...
            else if (type == "PointLight") handlePointLight(ctx.params);
            else if (type == "SpotLight") handleSpotLight(ctx.params);
            else if (type == "DirectionalLight") handleDirectionalLight(ctx.params);
            else if (type == "PerspectiveCamera") handlePerspectiveCamera(ctx.params);
            else if (type == "Sphere") handleSphere(ctx.params);
            else if (type == "Cube") handleCube(ctx.params);
            else if (type == "Cone") handleCone(ctx.params);
            else if (type == "Cylinder") handleCylinder(ctx.params);
            else if (type == "IndexedFaceSet") handleIndexedFaceSet(ctx.params);
            else if (type == "Coordinate3") handleCoordinate3(ctx.params);
            else if (type == "Normal") handleNormal(ctx.params);
            else if (type == "Translation") handleTranslation(ctx.params);
            else if (type == "Rotation") handleRotation(ctx.params);
            else if (type == "Scale") handleScale(ctx.params);
            else if (type == "Transform") handleTransform(ctx.params);
            else if (type == "Material") handleMaterial(ctx.params);
            else {
                cout << "VRML1 applyProperties, node " << type << " not handled!" << endl;
            }
        }
        ...
        void handleBracket(string bracket) {
            auto open = [&]() {
                if (ctx.currentNode) {
                    if (ctx.currentNode->obj) {
                        ctx.currentNode = ctx.currentNode->addChild(ctx.nextNodeType, ctx.nextNodeName);
                        auto obj = makeObject();
                        ctx.currentNode->obj = obj;
                        ctx.currentNode->parent->obj->addChild(obj);
                        ctx.nextNodeType = "Untyped";
                        ctx.nextNodeName = "Unnamed";
                        if (isGroupNode(ctx.currentNode->type)) ctx.state = HEAD;
                        else                                    ctx.state = BODY;
                    } else cout << "WARNING in VRML handleBracket: currentNode at opening bracket has no vr object" << endl;
        ...
        void handleToken(string token) {
            ...
            if (ctx.state == FIELD) {
                ...
                if (isNumber(token)) { ctx.params[ctx.field] += token+" "; return; }
                ctx.state = BODY; // don't return here
            }
            if (ctx.state == BODY) {
                //if (isNumber(token)) { handleNumber(token); return; }
                ctx.params[token] = ""; // must be a field
                ctx.field = token;
                ctx.state = FIELD;
                return;
            }
