...
PyMethodDef VRPySound::methods[] = {
    ...
    {"synthBuffer", PyWrapOpt(Sound, synthBuffer, "synthBuffer( [[f,A]], [[f,A]], T, maxQueue )\t\n [f,A] frequency/amplitude pairs, interpolate the two spectra, T is the packet duration in seconds", "-1", vector<short>, vector<Vec2d>, vector<Vec2d>, float, int) },
    {"synthBufferOnChannels", PyWrapOpt(Sound, synthBufferOnChannels, "synthBufferOnChannels( [[[f,A]]], [[[f,A]]], T, maxQueue )\n\t [[f,A]] list of channels with each containing a list of frequency/amplitude pairs in channel order, interpolate the two spectra\n\tT is the packet duration in seconds\n\t", "-1", void, vector<vector<Vec2d>>, vector<vector<Vec2d>>, float, int) },
    ...
    {"getBuffer", PyWrap(Sound, getBuffer, "Get queued packets in single buffer", vector<float>) },
    {"getDuration", PyWrap(Sound, getDuration, "Get sound duration in seconds", double) },
#ifdef _WIN32
...
PyMethodDef VRPySoundManager::methods[] = {
    {"setupSound", PyWrapOpt(SoundManager, setupSound, "Play sound, (path, doLoop = False, doPlay = False), lopping and playing are optional", "0|0", VRSoundPtr, string, bool, bool) },
