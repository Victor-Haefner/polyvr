...
PyMethodDef VRPyPath::methods[] = {
    ...
    {"getPose", PyWrapOpt2(Path, getPose, "Return the pose at the spline coordinate t between point i and j - pose getPose( float t | int i, int j, bool fast)", "0|0|1", PosePtr, float, int, int, bool ) },
    {"getUniformPose", PyWrapOpt2(Path, getUniformPose, "Return the pose at the uniform path coordinate t between point i and j - pose getUniformPose( float t | int i, int j, bool fast)", "0|0|1", PosePtr, float, int, int, bool ) },
    ...
    {"getSegmentIDs", PyWrap2(Path, getSegmentIDs, "Return the two IDs of the points before and after t, t from 0 to 1", Vec2i, double ) },
