...
bool VRIntersect::dragCB(VRTransformWeakPtr caster, VRObjectWeakPtr tree, VRDeviceWeakPtr dev) {
    if (!dnd) return true;
...
VRDeviceCbPtr VRIntersect::addDrag(VRTransformWeakPtr caster, VRObjectWeakPtr tree) {
    ...
    VRDeviceCbPtr fkt = VRDeviceCb::create("Intersect_drag", bind(&VRIntersect::dragCB, this, caster, tree, _1));
    dra_fkt_map[key] = fkt;
    return fkt;
}
