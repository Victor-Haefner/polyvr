...
/* For 64bit conversation void* to int fails in several places
 * replace L1191
 * #define OS_SOCKET(a)  ((int )(a))
 * by
	...
	do{
		...
		if(!strncmp(s, "6d ", 3)){
			...
			for(i=0; i<n; i++){                           // get data of standard bodies
				if(!(s = string_get_block(s, (char *)"if", &id, &f))){
					return false;
				}
				...
				if(!(s = string_get_block(s, (char *)"fff", NULL, act_body[id].loc))){
					return false;
				}

				if(!(s = string_get_block(s, (char *)"fffffffff", NULL, act_body[id].rot))){
					return false;
				}
			}
		...
		if(!strncmp(s, "6df ", 4)){
			...
			for(i=0; i<n; i++){                           // get data of Flysticks
				if(!(s = string_get_block(s, (char *)"ifi", iarr, &f))){
					return false;
				}
				...
				if(!(s = string_get_block(s, (char *)"fff", NULL, act_flystick[i].loc))){
					return false;
				}

				if(!(s = string_get_block(s, (char *)"fffffffff", NULL, act_flystick[i].rot))){
					return false;
				}
			}
		...
		if(!strncmp(s, "6df2 ", 5)){
			...
			for(i=0; i<n; i++){                           // get data of Flysticks
				if(!(s = string_get_block(s, (char *)"ifii", iarr, &f))){
					return false;
				}
		...
		if(!strncmp(s, "6dmt ", 5)){
			...
			for(i=0; i<n; i++){                           // get data of measurement tools
				...
				if(!(s = string_get_block(s, (char *)"fff", NULL, act_meatool[i].loc))){
					return false;
				}

				if(!(s = string_get_block(s, (char *)"fffffffff", NULL, act_meatool[i].rot))){
					return false;
				}
			}
		...
		if(!strncmp(s, "gl ", 3)){
			...
			for(i=0; i<n; i++){                           // get data of hands
				...
				if(!(s = string_get_block(s, (char *)"fff", NULL, act_hand[id].loc))){
					return false;
				}

				if(!(s = string_get_block(s, (char *)"fffffffff", NULL, act_hand[id].rot))){
					return false;
				}

				for(j=0; j<act_hand[id].nfinger; j++){     // get data of fingers
					if(!(s = string_get_block(s, (char *)"fff", NULL, act_hand[id].finger[j].loc))){
						return false;
					}

					if(!(s = string_get_block(s, (char *)"fffffffff", NULL, act_hand[id].finger[j].rot))){
						return false;
					}

					if(!(s = string_get_block(s, (char *)"ffffff", NULL, farr))){
						return false;
					}
		...
		if(!strncmp(s, "3d ", 3)){
			...
			for(i=0; i<act_num_marker; i++){              // get data of single markers
				if(!(s = string_get_block(s, (char *)"if", &act_marker[i].id, &act_marker[i].quality))){
					return false;
				}

				if(!(s = string_get_block(s, (char *)"fff", NULL, act_marker[i].loc))){
					return false;
				}
			}
...
//#ifdef OS_UNIX
#define OS_SOCKET(a)  ((long )(a))    // convert general socket to Unix socket
