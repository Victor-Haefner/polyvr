...
inline static float length(const csgjs_vector & a) { return sqrt(dot(a, a)); }
...
csgjs_vertex csgjs_sphereVertex(float radius, float theta, float phi)
{
	theta *= M_PI * 2;
	phi *= M_PI;
	csgjs_vector dir = csgjs_vector(cos(theta) * sin(phi),
									cos(phi),
									sin(theta) * sin(phi));
	csgjs_vertex vertex;
	vertex.pos = dir * radius;
	vertex.normal = dir;
	return vertex;
}

csgjs_model csgjs_sphere(const float radius, const int slices, const int stacks)
{
	std::vector<csgjs_polygon> polygons;

	// Avoid having to cast stacks and slices for every division in the loop
	const float fslices = (float)slices;
	const float fstacks = (float)stacks;
	...
	for(int i = 0; i < slices; i++)
	{
		...
		{
			std::vector<csgjs_vertex> vertices;
			vertices.push_back(csgjs_sphereVertex(radius, (float)i/fslices, (float)j/fstacks));

			if(j > 0)
				vertices.push_back(csgjs_sphereVertex(radius, (float)(i+1)/fslices, (float)j/fstacks));
			if(j < stacks-1)
				vertices.push_back(csgjs_sphereVertex(radius, (float)(i+1)/fslices, (float)(j+1)/fstacks));

			vertices.push_back(csgjs_sphereVertex(radius, (float)i/fslices, (float)(j+1)/fstacks));
...
inline csgjs_vertex csgjs_cubeVertex(csgjs_vector pos, csgjs_vector normal)
{
	csgjs_vertex vertex;
	vertex.pos = pos;
...
csgjs_model csgjs_cube(const int size)
{
	int fsize = (float)size;

	std::vector<csgjs_vector> positions;
	positions.push_back(csgjs_vector(-fsize/2.0, -fsize/2.0, fsize/2.0));
	positions.push_back(csgjs_vector(fsize/2.0, -fsize/2.0, fsize/2.0));
	positions.push_back(csgjs_vector(fsize/2.0, -fsize/2.0, -fsize/2.0));
	positions.push_back(csgjs_vector(-fsize/2.0, -fsize/2.0, -fsize/2.0));
	positions.push_back(csgjs_vector(-fsize/2.0, fsize/2.0, fsize/2.0));
	...
	std::vector<csgjs_vector> normals;
	normals.push_back(csgjs_vector( 1,0,0)); //0
	normals.push_back(csgjs_vector(-1,0,0)); //1
	normals.push_back(csgjs_vector(0, 1,0)); //2
	normals.push_back(csgjs_vector(0,-1,0)); //3
	normals.push_back(csgjs_vector(0,0, 1)); //4
	normals.push_back(csgjs_vector(0,0,-1)); //5

	std::vector<csgjs_vertex> vertices;
	// front
	vertices.push_back(csgjs_cubeVertex(positions[0], normals[4]));
	vertices.push_back(csgjs_cubeVertex(positions[1], normals[4]));
	vertices.push_back(csgjs_cubeVertex(positions[5], normals[4]));
	vertices.push_back(csgjs_cubeVertex(positions[4], normals[4]));
	// left
	vertices.push_back(csgjs_cubeVertex(positions[3], normals[1]));
	vertices.push_back(csgjs_cubeVertex(positions[0], normals[1]));
	vertices.push_back(csgjs_cubeVertex(positions[4], normals[1]));
	vertices.push_back(csgjs_cubeVertex(positions[7], normals[1]));
	// back
	vertices.push_back(csgjs_cubeVertex(positions[2], normals[5]));
	vertices.push_back(csgjs_cubeVertex(positions[3], normals[5]));
	vertices.push_back(csgjs_cubeVertex(positions[7], normals[5]));
	vertices.push_back(csgjs_cubeVertex(positions[6], normals[5]));
	// right
	vertices.push_back(csgjs_cubeVertex(positions[1], normals[0]));
	vertices.push_back(csgjs_cubeVertex(positions[2], normals[0]));
	vertices.push_back(csgjs_cubeVertex(positions[6], normals[0]));
	vertices.push_back(csgjs_cubeVertex(positions[5], normals[0]));
	// top
	vertices.push_back(csgjs_cubeVertex(positions[4], normals[2]));
	vertices.push_back(csgjs_cubeVertex(positions[5], normals[2]));
	vertices.push_back(csgjs_cubeVertex(positions[6], normals[2]));
	vertices.push_back(csgjs_cubeVertex(positions[7], normals[2]));
	// bottom
	vertices.push_back(csgjs_cubeVertex(positions[4], normals[3]));
	vertices.push_back(csgjs_cubeVertex(positions[3], normals[3]));
	vertices.push_back(csgjs_cubeVertex(positions[2], normals[3]));
	vertices.push_back(csgjs_cubeVertex(positions[1], normals[3]));
