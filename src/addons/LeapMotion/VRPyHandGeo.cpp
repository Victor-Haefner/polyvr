...
PyMethodDef VRPyHandGeo::methods[] = {
    ...
    {"connectToLeap", (PyCFunction)VRPyHandGeo::connectToLeap, METH_VARARGS, "Add description" },
    {NULL}  /* Sentinel */
};
...
PyObject* VRPyHandGeo::connectToLeap(VRPyHandGeo* self, PyObject* args) {
    if (!self->valid()) return NULL;
    VRPyLeap* leap = 0;
    if (!PyArg_ParseTuple(args, "O", &leap)) return NULL;
    self->objPtr->connectToLeap(leap->objPtr);
    Py_RETURN_TRUE;
}
