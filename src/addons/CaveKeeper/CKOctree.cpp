...
CKOctree::element::element(CKOctree* tree, Vec3d p, Vec3i otp, int s) : pos(p), otpos(otp), size(s), _size(s / 2) {
    for (int i=0;i<8;i++) {
        children[i] = 0;
        octIsEmpty[i] = 0;
        float k = 0.2;
        if (i<6) vertexLight[i] = Vec4d(k,k,k,k);
    }
...
void CKOctree::element::add(element* e) {
    ...
    if (e->size*2 != size) {
        ...
        if (c == 0) {
            ...
            Vec3d p = (Vec3d(otp) - Vec3d(1,1,1))*0.5 ;
...
int CKOctree::element::getOctant(Vec3i p) { return getOctant(Vec3d(p)); }
int CKOctree::element::getOctant(Vec3d p) {
    if(p[0] >= 0) {
        if(p[1] >= 0) {
            if(p[2] >= 0) return 0;
            else return 1;
...
bool CKOctree::element::inside(Vec3d f) {
    for (int i=0;i<3;i++) {
...
CKOctree::element* CKOctree::add(Vec3i _p) {
    ...
    if (root == 0) root = new element(this, Vec3d(-0.5, -0.5, -0.5), Vec3i(), s);

    if (root->size < s) {
        element* tmp = root;
        root = new element(this, Vec3d(-0.5, -0.5, -0.5), Vec3i(), s);
        for (int i=0;i<8;i++) root->add(tmp->children[i]);
        delete tmp;
    }

    element* cube = new element(this, Vec3d(_p), p, 1);
    cube->leaf = true;
    root->add(cube);
    cube->updateLightning(lightTree->radiusSearch(Vec3d(_p), 10));
    N++;
...
void CKOctree::element::updateLightning(CKOctree::light* l) {
    ...
    normals[2]= Vec3d(0,0,1);
    normals[3]= Vec3d(-1,0,0);
    normals[4]= Vec3d(0,-1,0);
    normals[5]= Vec3d(0,0,-1);

    Vec3d tangents[6];
    tangents[0] = Vec3d( 0, 1, 0);
    tangents[1] = Vec3d( 0, 0, 1);
    tangents[2] = Vec3d( 1, 0, 0);
    tangents[3] = Vec3d( 0, 1, 0);
    tangents[4] = Vec3d( 0, 0, 1);
    tangents[5] = Vec3d( 1, 0, 0);
    ...
    for (int j=0;j<6;j++) {
        ...
        for (int k=0;k<4;k++) {
            ...
            double li = min(0.1+2.0*cos/a, 2.0);
...
void CKOctree::setEmpty(Vec3i p) {
    element* e = get(Vec3d(p));
    if (e == 0) return;

    int o = e->getOctant(Vec3d(p) - e->pos);
    e->octIsEmpty[o] = true;
}

//get the smallest element at that position
CKOctree::element* CKOctree::get(Vec3d p, CKOctree::element* e) {
    if (e == 0) e = root;
    if (e == 0) return 0;
...
CKOctree::element* CKOctree::get(Line ray, CKOctree::element* e, string indent) {
    ...
    Vec3d pos = Vec3d(ray.getPosition());
    hitPoint = pos;
    hitElement = e;
    hitNormal = pos - e->pos;
    for (int i=0;i<3;i++) {
    ...
    Vec3d dir = Vec3d(ray.getDirection());
    Vec3d epos = e->pos;
    Vec3d diff = pos - epos;
    ...
    for (int i=0; i<3;i++) {
        ...
        Vec3d r = pos + dir*k; //intersection point
    ...
    for (itr = octants.begin(); itr != octants.end(); itr++) {
        ...
        if (c != 0) {
            ray.setValue(Pnt3f(ipoints[itr->second]), Vec3f(dir));
            element* res = get(ray, c, indent + " ");
...
vector<CKOctree::element*> CKOctree::getAround(Vec3d pos, float r) {
...
Vec3d CKOctree::getHitPoint() { return hitPoint; }
Vec3d CKOctree::getHitNormal() { return hitNormal; }
