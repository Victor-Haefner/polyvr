...
The traffic simulator is running a HTTP server that receives and sends JSON data. The PolyVR framework can connect to the server over this interface.
...
send back, might contain the requested data or might contain an error message. If an empty structure is returned there has been no error, it might
...
too big or too small (e.g. a laneWidth of -2.5) they will be clamped to useful values.

IDs that are generated by the simulator are at least 256.

Note that the used ids are bound to the class, e.g. a vehicle and a viewarea can have the same id while two viewareas can not.
...
After starting the server is in the \c paused state and accepts input data like nodes, streets, vehicle and driver types.
...
\li \c paused: The server is paused (e.g. is not moving the vehicles) and can be set to \c running again.

The server state can be set to:
\li \c running: The server is running the simulation until another state is set by the client.
\li \c paused: The server is paused (e.g. is not moving the vehicles) and can be set to \c running again.
\li \c restart: the server will drop all stored data and will return to the paused state. This will be the same as restarting the server process.
\li \c shutdown: The server will drop all stored data and will quit its process.

After setting the server state to restart or shutdown, only the packet with this message will be processed and all further packets are ignored.
...
HTTP error codes:
\li 200 Ok: No error, the data has been received and could be parsed without problems. The body contains a JSON structure.
...
\li 400 Bad Request: The POST data has been send to the right file but the data could not be parsed to a JSON structure or the GET request has not set one of the recognized fields.

JSON error codes:
Errors occurring while working on the supplied data are reported in the returned JSON data. For this, two fields might be present in the response: "error" and "error_message".
If the error field is not set or is \c null no error has occurred. Otherwise the error field is set to a short string describing the error type while
...
Normally, only one or a few of the commands in each subsection are used at the same time, for example you do not have to set the
...
The server state can be modified by setting the "serverState" field, either as a single packet or as part of some other input data.
...
@subsection network_doc_input_map Add and remove nodes and streets
    ...
    "addStreets": [
            {
                ...
                // the middle lane can change to its right lane and to the lane of
                ...
                // (optional) The type of the street, e.g. a highway or in a living area.
                ...
                // middle lane can turn to left and drive through,
...
@subsection network_doc_input_vehicles Add and remove vehicle and driver types
    ...
    "addDriverTypes" : [
            {
                ...
                // or driving over a red traffic light
                "lawlessness": 0.1,
                // Probability [0-1] for the driver to behave responsible in traffic,
                // e.g. lane changing or safety margin to other cars (0.9 = big margin)
                "cautiousness": 0.9
            },
            {
                "id": 32,
...
simulation in a VR-environment. To change and retrieve the data in a certain area, a view area can be set and moved.
After adding the area, the positions of the vehicles in that area can be retrieved with a GET request (see below).

\verbatim
{
    // Creates a viewarea at a given position (e.g. a traffic camera or the background outside a flat)
    // or following a vehicle around, depending on the provided parameters
    "addViewareas": [
            {
                // The id of this area
                "id": 1,
...
\verbatim
{
    ...
    "id": 5,
    // The list of vehicles in the area with their positions and orientations.
    ...
    "vehicles": [
            {
                ...
                "dAngle": [ 0, 2.1, 0, 0.04 ],
                // (optional) The state of the vehicle, e.g. turning lights or a possible collision
    ...
    "trafficlights": [
            {
                ...
                // which are red, red and green; enumerated from left to right.
                ...
                // Possible values are "red", "amber", "green" and "redamber"
                "state": [ "red", "red", "green" ]
            }
        ]
}
