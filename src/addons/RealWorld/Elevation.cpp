...
int Elevation::getSomeElevation(float lat, float lon){
    lat = roundFloat(lat);
    lon = roundFloat(lon);
...
/** not finished => to do **/
void Elevation::readElevationFile(string id){
...
void Elevation::createElevationFile(string id, string latlons[]){
...
Json::Value Elevation::interpolateElevationJson(Json::Value rb){
    return Json::Value();
}

/** get String with Json Data for lat- lon pair && their elevation && distance **/
string Elevation::getMapQuestElevations(string latlons){
        CURL *curl;
        CURLcode res;
        string readBuffer;
...
Json::Value Elevation::getBetterJson(string readBuffer[], Json::Value event){
    string rb, sLat, sLon;
    for(int i = 0; i< n; i++){
        rb = readBuffer[i];
...
string Elevation::getLatLonName(float lat, float lon){
...
*/
bool Elevation::fileExists(const std::string& filename)
{
    struct stat buf;
...
size_t Elevation::WriteCallback(void *contents, size_t size, size_t nmemb, void *userp)
{
...
/** trim from end **/
string Elevation::trim(string s) {
        s.erase(s.find_last_not_of(" \n\r\t")+1);
        return s;
}

/** round float numbers to multiple of 0.0001 **/
float Elevation::roundFloat(float f){
    f += 0.00005; //for exact rounding
    f = (float)(int)(f * 10000);
    return f/10000;
}

float Elevation::cutFloat(float f, int p){
    stringstream ss;
...
float Elevation::toFloat(string s){
    stringstream sstr;
    float f;
    sstr << s;
    sstr >> f;
...
char* Elevation::toCharArr(float f){
    std::stringstream os;
    os << f;
    return const_cast<char*>(os.str().c_str());
}

string Elevation::toFloatString(float f, int precision){
    std::stringstream ss;
