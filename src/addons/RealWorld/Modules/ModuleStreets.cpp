...
ModuleStreets::ModuleStreets(bool t, bool p) : BaseModule("ModuleStreets", t,p) {
    ...
    signTCs["noEntry"] = Vec4d(0.6242,0.6242+0.0385,1.0-0.5766-0.0559,1.0-0.5766);
    signTCs["straightOrRight"] = Vec4d(0.7324,0.7324+0.0348,1.0-0.5766-0.0519,1.0-0.5766);
}
...
void ModuleStreets::loadBbox(MapGrid::Box bbox) {
    ...
    for (auto node : osmMap->getNodes()) { // Load StreetJoints
        Vec2d pos = mc->realToWorld(Vec2d(node.second->lat, node.second->lon));
...
void ModuleStreets::makeStreetLight(StreetSegment* seg, VRGeoData* geo) {
    if (seg->name == "") return;
    Vec3d pA = elevate(seg->jointA->position, 0);
    Vec3d pB = elevate(seg->jointB->position, 0);
    Vec3d D = pA-pB;
    float DL = D.length();
    ...
    for (int i=0; i<N; i++) {
        ...
        Pose p(pB+D*i/N-X*k, X*k, Vec3d(0,1,0));
        StreetLamp::add(p, geo);
    }
}

void ModuleStreets::makeStreetSign(Vec3d p, string name, VRGeoData* geo) {
...
void ModuleStreets::makeStreetNameSign(StreetSegment* seg, VRAnnotationEnginePtr ae) {
    ...
    Vec3d p(p2[0], h1, p2[1]);
    ae->add(p, name);
}

void ModuleStreets::makeSegment(StreetSegment* s, map<string, StreetJoint*>& joints, VRGeoData* streets, VRGeoData* signs) {
    Vec2d leftA = Vec2d(s->leftA);
    Vec2d rightA = Vec2d(s->rightA);
    Vec2d leftB = Vec2d(s->leftB);
    Vec2d rightB = Vec2d(s->rightB);
    ...
    if (!s->bridge) {//normal street
        for (int l = 0; l < s->lanes; l++) {
            ...
            pushStreetQuad( lA, rA, rB, lB, Vec3d(0,1,0), streets, false, Vec3d(k1,k2,1) );
        }
        return;
    }
    ...
    auto pushPart = [&](Vec2d height, int i) {
        Vec2d lA = leftA + ABPart*i;
        Vec2d rA = rightA + ABPart*i;
        Vec3d a1 = elevate(lA, height[0]);
        Vec3d a2 = elevate(rA, height[0]);
        Vec3d b1 = elevate(lA + ABPart, height[1]);
        Vec3d b2 = elevate(rA + ABPart, height[1]);

        Vec3d normal = (a2-a1).cross(b1-a1);
        for (int l = 0; l < s->lanes; l++) {
            auto lA = a1+laneW3*l;
            auto lB = b1+laneW3*l;
            ...
            pushStreetQuad( lA, rA, rB, lB, -normal, streets, false, Vec3d(k1,k2,1) );
        }
        pushStreetQuad(a1-th, a2-th, b2-th, b1-th, normal, streets, false, Vec3d(0.75,1,1) );
        pushStreetQuad(a1, a1-th, b1-th, b1, Vec3d(-(b1-a1)[2], 0, (b1-a1)[0]), streets, true); //side1
        pushStreetQuad(a2, a2-th, b2-th, b2, Vec3d((b2-a2)[2], 0, -(b2-a2)[0]), streets, true); //side2
    };

    if (!s->jointA || !s->jointB) return;
    Vec4d heights = Vec4d(high, high, high, high);
    if (s->jointA->bridge && !s->jointB->bridge) heights = Vec4d(streetH, high*0.3, high*0.7, high);
    if (!s->jointA->bridge && s->jointB->bridge) heights = Vec4d(high, high*0.7, high*0.3, streetH);

    pushPart(Vec2d(heights[0], heights[1]), 0); // start
    pushPart(Vec2d(heights[1], heights[2]), 1); // middle
    pushPart(Vec2d(heights[2], heights[3]), 2); // end

    // Idee: baue eine maximale rampenlänge ein
    // Idee: berechne Steigung und entscheide dementsprechend fuer Treppen
}

void ModuleStreets::pushQuad(Vec3d a1, Vec3d a2, Vec3d b2, Vec3d b1, Vec3d normal, VRGeoData* geo, Vec4d tc) {
    pushQuad(a1,a2,b2,b1,normal,geo,Vec2d(tc[0], tc[2]), Vec2d(tc[1], tc[2]), Vec2d(tc[1], tc[3]), Vec2d(tc[0], tc[3]));
}

void ModuleStreets::pushStreetQuad(Vec3d a1, Vec3d a2, Vec3d b2, Vec3d b1, Vec3d normal, VRGeoData* geo, bool isSide, Vec3d tc) {
    ...
    else pushQuad(a1,a2,b2,b1,normal,geo,Vec2d(tc[0], 0), Vec2d(tc[1], 0), Vec2d(tc[1], len), Vec2d(tc[0], len));
}

void ModuleStreets::pushQuad(Vec3d a1, Vec3d a2, Vec3d b2, Vec3d b1, Vec3d normal, VRGeoData* geo, Vec2d tc1, Vec2d tc2, Vec2d tc3, Vec2d tc4) {
...
void ModuleStreets::pushTriangle(Vec3d a1, Vec3d a2, Vec3d c, Vec3d normal, VRGeoData* geo, Vec2d t1, Vec2d t2, Vec2d t3 ) {
...
Vec3d ModuleStreets::elevate(Vec2d p, float h) {
    auto mc = RealWorld::get()->getCoordinator();
    if (!mc) return Vec3d(p[0], 0, p[1]);
    return Vec3d(p[0], mc->getElevation(p) + h, p[1]);
}
...
void ModuleStreets::makeCurve(StreetJoint* sj, map<string, StreetSegment*>& streets, map<string, StreetJoint*>& joints, VRGeoData* geo) {
    ...
    if (jp1->leftExt != _NULL && jp1->leftExt != _NULL) {
        Vec3d r1,l1,r2,l2,e1,e2,d1,d2,d3;
        ...
        for (int i=0; i<Nl; i++) {
            ...
            int t = -(Nl-i-1);
            pushStreetQuad(r1 + d1*t, l1 + d1*i, e1 + d3*i, e2 + d3*t, norm, geo, false, Vec3d(k2, k1, 1) );
            pushStreetQuad(r2 + d2*i, l2 + d2*t, e2 + d3*t, e1 + d3*i, norm, geo, false, Vec3d(k1, k2, 1) );
        }
    }
}
...
void ModuleStreets::makeJoint(StreetJoint* sj, map<string, StreetSegment*>& streets, map<string, StreetJoint*>& joints, VRGeoData* geo) {
    ...
    Vec2d _NULL2;
    Vec3d norm = Vec3d(0, 1, 0);
    Vec3d middle = elevate(sj->position, jointH);
    Vec2d tm = Vec2d(0.875, 0.5);
    ...
    for (JointPoints* jp : jointPoints) {
        ...
        pushTriangle(right, left, middle, norm, geo, Vec2d(0, 1), Vec2d(1, 1), tc05);
    ...
    pushTriangle(firstRight, rightExt, rightExt+Vec3d(0, bS, 0), normal, geo, tc05, tc05, tc05);
    pushTriangle(rightExt+Vec3d(0, bS, 0), firstRight+Vec3d(0, bS, 0), firstRight, normal, geo, tc05, tc05, tc05);
}
...
void ModuleStreets::makeJoint31(StreetJoint* sj, map<string, StreetSegment*>& streets, map<string, StreetJoint*>& joints, VRGeoData* geo, VRGeoData* signs2) {
    ...
    Vec3d r3 = elevate(jp3->right, jointH);
    Vec3d l3 = elevate(jp3->left , jointH);
    Vec3d s = elevate(sj->position, jointH) - r3+(r3-l3)*0.5;
    Vec3d s3D = (r3-l3)*1/3.0;
    pushStreetQuad(r3, l3, l3+s, r3+s, n, geo, false, Vec3d(0, 0.75, 3));

    makeStreetSign(elevate(sj->position, jointH), "straightOrRight", signs2);

    for (int i=0; i<3; i++) {
        JointPoints* jp = jointPoints[(sL3+1+i)%4];
        Vec3d pr = elevate(jp->right, jointH);
        Vec3d pl = elevate(jp->left, jointH);
        pushStreetQuad(pr, pl, l3+s+s3D*(i+1), l3+s+s3D*i, n, geo, false, Vec3d(0.75, 1, 1));
    }
